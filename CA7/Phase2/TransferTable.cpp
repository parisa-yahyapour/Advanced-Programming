...
void TransferTable::apply_transfers(std ::vector<std ::shared_ptr<Player>> &fantasy_team_players, std ::shared_ptr<Player> &captain)
{
    for (int index_transfer = 0; index_transfer < transfers_informations.size(); index_transfer++)
    {
        ...
        else
        {
            for (int index_player = 0; index_player < fantasy_team_players.size(); index_player++)
            {
                ...
                {
                    if (fantasy_team_players[index_player] == captain)
                    {
                        captain = NULL;
                    }
...
std ::shared_ptr<Player> TransferTable::find_player_ptr_in_transfer_history(std ::string name)
{
    for (int index = 0; index < transfers_informations.size(); index++)
    {
        if (transfers_informations[index].first->check_name(name))
        {
            return transfers_informations[index].first;
        }
    }
    return NULL;
}

void TransferTable::save_captain_data(shared_ptr<Player> selected_player)
{
    new_captain = selected_player;
}