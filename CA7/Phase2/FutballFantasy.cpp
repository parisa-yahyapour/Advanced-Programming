...
void FutballFantasy ::handle_get_command(vector<string> arguments)
{
    ...
    case CODE_SQUAD:
        ...
        break;
    case CODE_SHOW_BUDGET:
        this->show_budget(arguments);
        break;
    }
}
...
void FutballFantasy ::handle_post_command(vector<string> arguments)
{
    ...
    case CODE_PASS_WEEK:
        ...
        OutputHandler(OK).handle_function_result();
        break;
    case CODE_SET_CAPTAIN:
        this->set_captian(arguments);
        OutputHandler(OK).handle_function_result();
        break;
    }
}
...
void FutballFantasy ::save_premier_data_in_database(vector<string> columns)
{
    ...
    for (int index_column = 1; index_column < columns.size(); index_column++)
    {
        ...
        {
            vector<string> name_price_player = seperated_name_price_player(one_columns_data[i]);
            shared_ptr<Player> new_player;
            switch (index_column)
            {
            case GLOALKEEPER:
            {
                new_player = make_shared<GoalKeeper>(name_price_player, index_column);
            }
            break;
            case DEFENDER:
            {
                new_player = make_shared<Defender>(name_price_player, index_column);
            }
            break;
            case MIDFIELDER:
            {
                new_player = make_shared<MidFielder>(name_price_player, index_column);
            }
            break;
            case FORWARD:
            {
                new_player = make_shared<Forward>(name_price_player, index_column);
            }
            break;
            }
...
vector<string> FutballFantasy::seperated_name_price_player(string column_data)
{
    vector<string> name_price;
    vector<string> seperated_name_price = separate_by_delimiter(column_data, DELIMITERS[DELIMITER_NUMBERS]);
    name_price.push_back(seperated_name_price[0]);
    vector<string> seperated_price = separate_by_delimiter(seperated_name_price[1], DELIMITERS[DELIMITER_PRICE]);
    name_price.push_back(seperated_price[0]);
    return name_price;
}
...
void FutballFantasy ::save_week_data_in_database(vector<string> columns, int num_week, vector<shared_ptr<Match>> &week_matches)
{
    ...
    shared_ptr<RealTeam> team1 = database.find_team_by_name(name_teams[0]);
    shared_ptr<RealTeam> team2 = database.find_team_by_name(name_teams[1]);
    pair<shared_ptr<RealTeam>, shared_ptr<RealTeam>> team1_team2;
    ...
    save_team_players_presence(team1_team2, columns, num_week);
    save_goals_goals_assist(team1_team2, columns[GOALS_WITH_ASSOST], num_week);
    apply_effect_win_lose_equal(team1_team2, goal_team1_team2, num_week);
    calculate_players_score(team1_team2, goal_team1_team2, num_week);
    shared_ptr<Match> new_match = make_shared<Match>(team1_team2, goal_team1_team2);
    week_matches.push_back(new_match);
}

void FutballFantasy::calculate_players_score(pair<shared_ptr<RealTeam>, shared_ptr<RealTeam>> team1_team2, pair<int, int> goal_team1_team2, int num_week)
{
    team1_team2.first->calculate_real_team_player_week_score(num_week, goal_team1_team2.second);
    team1_team2.second->calculate_real_team_player_week_score(num_week, goal_team1_team2.first);
}

void FutballFantasy::apply_effect_win_lose_equal(pair<shared_ptr<RealTeam>, shared_ptr<RealTeam>> team1_team2, pair<int, int> goal_team1_team2, int num_week)
{
    if (goal_team1_team2.first > goal_team1_team2.second)
    {
        team1_team2.first->apply_win_effect(num_week);
        team1_team2.second->apply_loose_effect(num_week);
    }
    else if (goal_team1_team2.second > goal_team1_team2.first)
    {
        team1_team2.second->apply_win_effect(num_week);
        team1_team2.first->apply_loose_effect(num_week);
    }
    else
    {
        team1_team2.first->apply_equal_effect(num_week);
        team1_team2.second->apply_equal_effect(num_week);
    }
}

void FutballFantasy::save_goals_goals_assist(pair<shared_ptr<RealTeam>, shared_ptr<RealTeam>> team1_team2, string goals_info, int num_week)
{
    vector<string> seperated_each_goal_data = separate_by_delimiter(goals_info, DELIMITERS[DELIMITER_EACH_COLUMN_DATA]);
    for (int index_goal = 0; index_goal < seperated_each_goal_data.size(); index_goal++)
    {
        vector<string> goal_and_assist = separate_by_delimiter(seperated_each_goal_data[index_goal], DELIMITERS[DELIMITER_NUMBERS]);
        shared_ptr<Player> score_goal = database.find_player_by_name(goal_and_assist[0]);
        if (goal_and_assist[1] == OWN_GOAL)
        {
            score_goal->save_own_goal_for_player(num_week);
        }
        else
        {
            shared_ptr<Player> assist_goal = database.find_player_by_name(goal_and_assist[1]);
            score_goal->score_goal_for_player(num_week);
            assist_goal->assist_goal_for_player(num_week);
            if (team1_team2.first->find_player_by_name_in_team(goal_and_assist[0]) != NULL)
            {
                team1_team2.second->add_post_that_attack(score_goal->post_player_in_current_match());
            }
            else
            {
                team1_team2.first->add_post_that_attack(score_goal->post_player_in_current_match());
            }
        }
    }
}

void FutballFantasy::save_team_players_presence(pair<shared_ptr<RealTeam>, shared_ptr<RealTeam>> team1_team2, const std::vector<string> &columns, int num_week)
{
    vector<string> players_team_1 = separate_by_delimiter(columns[PLAYERS_TEAM1], DELIMITERS[DELIMITER_EACH_COLUMN_DATA]);
    vector<string> players_team_2 = separate_by_delimiter(columns[PLAYERS_TEAM2], DELIMITERS[DELIMITER_EACH_COLUMN_DATA]);
    team1_team2.first->update_players_presence_in_week(num_week, players_team_1);
    team1_team2.second->update_players_presence_in_week(num_week, players_team_2);
}
...
void FutballFantasy ::get_players(const vector<string> &arguments)
{
    ...
    shared_ptr<RealTeam> wanted_team = database.find_team_by_name(team_name);
    if (wanted_team == NULL)
    {
        throw(ErrorHandler(NOT_FOUND));
    }
    ...
    wanted_team->calculate_goal_assist_clean_sheets_team_players(num_week);
...
void FutballFantasy ::set_captian(const vector<string> &inputs)
{
    check_allowable_request_for_user();
    if (inputs[KEYWORD1] != NAME_KEYWORD ||
        inputs[QUESTION_MARK] != DELIMITER_BETWEEN_OPERATION_ARGUMENTS ||
        inputs.size() < NUM_ARGUMENTS_SET_CAPTAIN)
    {
        throw(ErrorHandler(BAD_REQUEST));
    }
    string captain_name = EMPTY_STRING;
    captain_name = assign_set_captain_values(captain_name, inputs);
    current_user->set_captain(captain_name);
}

string FutballFantasy ::assign_set_captain_values(string &name, const vector<string> inputs)
{
    for (int index = 4; index < inputs.size(); index++)
    {
        name += inputs[index];
...
void FutballFantasy ::show_budget(const std ::vector<std ::string> &inputs)
{
    check_allowable_request_for_user();
    check_correctness_input_static_arguments(inputs, NUM_ARGUMENTS_SHOW_BUDGET, NO_ARGUMENT_VECTOR);
