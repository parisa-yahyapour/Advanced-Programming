...
void User ::eliminate_player(std ::string name)
{
    ...
    shared_ptr<Player> selected_player = user_team->find_player_by_name(name);
...
void User::set_captain(string name)
{
    ...
    {
        selected_player = user_team->find_player_in_transform_table(name);
        if (selected_player == NULL)
        {
            throw(ErrorHandler(NOT_FOUND));
        }
    }
    user_team->set_captain(selected_player);
}

void User::show_budget()
{
    OutputHandler().print_budget(budget);
}