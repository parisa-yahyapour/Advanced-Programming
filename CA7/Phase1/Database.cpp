...
shared_ptr<Team> DataBase ::find_team_by_name(string name, bool is_real_team)
{
    if (is_real_team)
    {
    ...
    {
        for (int index_team = 0; index_team < fantasy_teams.size(); index_team++)
        {
            if (fantasy_teams[index_team]->check_name(name))
            {
                return fantasy_teams[index_team];
            }
        }
    }
    return NULL;
